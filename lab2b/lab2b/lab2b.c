...
int main() {
    ...
    int count;
    int count_masked;
    XGpio leds, switch_buttons;
    int led_status,  switch_button_status;
    int get_bs;

    led_status = XGpio_Initialize(&leds, GPIO_DEVICE_ID);

    switch_button_status = XGpio_Initialize(&switch_buttons, GPIO_DEVICE_ID);

    XGpio_SetDataDirection(&leds, 1, 0x00);
    XGpio_SetDataDirection(&switch_buttons, 2, 0xFF);
    if(led_status != XST_SUCCESS) {
        xil_printf("Initialization failed");    
    }
    if(switch_button_status != XST_SUCCESS) {
        xil_printf("Initialization failed");
    }
    count = 0;
    while(1) {
